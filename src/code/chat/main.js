import ChatModel, { ChatStatus } from "@/database/models/chat"
...
export async function saveMessageToChat(chatID, senderId, content) {
  try {
...
export async function getChatMessages(id, userId) {
  try {
...
export async function changeStatus(id, stuatus) {
  try {
...
export async function acceptChat(adminId, chatId) {
  try {
...
export async function isChatActiveWithId(userId, chatId) {
  try {
    const chat = await ChatModel.findById(chatId)
    if (!chat) {
      throw AppError.notFound("Chat not found.")
...
export async function connectUser(token, chatId) {
  try {
    const chat = await ChatModel.findById(chatId)
    if (!chat) {
      throw AppError.notFound("Chat not found.")
...
export async function connectAdmin(token, chatId) {
  try {
    const chat = await ChatModel.findById(chatId)
    if (!chat) {
      throw AppError.notFound("Chat not found.")
...
function _formatMessages(chat, id) {
  // messages:
